...
url = paste0("https://www.b3.com.br/pesquisapregao/download?filelist=PR", datas, ".zip,")
tmp <- tempfile(fileext = ".zip")
utils::download.file(url = url, destfile = tmp, mode = "wb", quiet = TRUE)
unzip(tmp, exdir = "Raw")
setwd("Raw")
unzip(paste0("PR",datas,".zip"))
...
tic <- "PETR"
indices <- which(grepl(tic, ignore.case = TRUE, data0))
...
# final_2[final_2[,1]=="PETRB381",] # Selecionar específica
#
#
